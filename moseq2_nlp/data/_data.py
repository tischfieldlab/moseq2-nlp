        ...
        for supergroup in custom_groupings:
            ...
            for subg in subgroups:
                ...
                if subg in group_mapping:
                    print(
                        f'WARNING: subgroup "{subg}" from supergroup "{supergroup}" already registered to supergroup "{group_mapping[subg]}"! Omitting...'
                    )
                    continue
...
def get_embedding_representation(
    ...
    E = (
        doc_embedding.model.dv.vectors if dm < 2 else 0.5 * (doc_embedding.model0.dv.vectors + doc_embedding.model1.dv.vectors)
    )  # <-- This is the new way.
